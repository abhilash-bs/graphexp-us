...
//                        }
	...
	function get_graph_info(){
		traversal_source = document.getElementById("graphInstance").value;
	...
	function search_query() {
		// Preprocess query
		traversal_source = document.getElementById("graphInstance").value;
		let input_string_1 = $('#search_value').val().split(",")[0];
		let input_field_1 = $('#search_field').val().split(",")[0];
		let input_string_2 = $('#search_value').val().split(",")[1];
		let input_field_2 = $('#search_field').val().split(",")[1];
		let label_field = $('#label_field').val();
		let limit_field = $('#limit_field').val();
		let search_type = $('#search_type').val();
		//console.log(input_field)
		var filtered_string = input_string_1;//You may add .replace(/\W+/g, ''); to refuse any character not in the alphabet
		...
		if (input_field_1 !== "" && input_string_1 !== "") {
			has_str += ".has('" + input_field_1 + "',";
			switch (search_type) {
				case "eq":
					if (isInt(input_string_1)){
						has_str += input_string_1 + ")"
					} else {
						has_str += "'" + input_string_1 + "')"
					}
					has_str += ".has('" + input_field_2 + "',";
					if (isInt(input_string_2)){
						has_str += input_string_2 + ")"
					} else {
						has_str += "'" + input_string_2 + "')"
					}					
					break;
	...
	function run_websocket_request(gremlin_query,server_url,query_type,active_node,message,callback){
		...
		ws.onmessage = function (event){
			...
			if(!isInt(code) || code<200 || code>299) {
				...
				$('#messageArea').html("Error retrieving data. Data not found or internal server error.");
				return 1;
			}
